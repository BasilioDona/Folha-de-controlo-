...
async function gerarPDF() {
    ...
    // Ativamos a compressão interna nativa para garantir ficheiros leves em KB
    ...
    try {
        const html2canvasOptions = {
            scale: 2.5,                 // Nitidez ideal sem distorcer linhas finas
            useCORS: true, 
            backgroundColor: '#ffffff', 
            logging: false,
            letterRendering: false,     
            windowWidth: 1300,          // Mantém a largura estável para evitar o erro da miniatura no mobile

            onclone: function(clonedDoc) {
                const style = clonedDoc.createElement('style');
                style.innerHTML = `
                    /* Contentor principal limpo e sem sombras */
                    .folha-a4 {
                    ...
                    /* TIPOGRAFIA EQUILIBRADA (Negrito limpo, sem exageros) */
                    .folha-a4, .folha-a4 td, .folha-a4 th, .folha-a4 span, .folha-a4 input {
                    ...
                    /* BARRA CINZA MUITO FRACA NO CABEÇALHO (Zona do APV, Descrição, Validação...) */
                    .tabela-f-af th {
                        background-color: #f4f5f7 !important; /* Cinza pastel extremamente suave */
                        color: #000000 !important;
                    ...
                    /* Preservação das marcações amarelas originais do cabeçalho */
                    .tabela-f-af th:nth-child(5),
                    .tabela-f-af th:nth-child(6) {
                        background-color: #ffff00 !important;
                    }

                    /* REMOVE TOTALMENTE A BARRA CINZA APENAS NA PARTE DO SALDO INICIAL */
                    /* Procura por classes comuns ou trs que representem o saldo inicial */
                    .row-saldo-inicial, 
                    tr[id*="inicial"], 
                    tr[class*="inicial"] td,
                    .tabela-f-af tr:nth-child(2) td, /* Geralmente a primeira linha de dados após o th */
                    td:contains("Saldo Inicial") { 
                        background-color: #ffffff !important; /* Força fundo branco puro */
                        background: #ffffff !important;
                        border-top: 1px solid #cccccc !important;   /* Linha horizontal muito fraca */
                        border-bottom: 1px solid #cccccc !important;/* Linha horizontal muito fraca */
                        padding: 8px 4px !important;
                    }

                    /* BARRA CINZA MUITO FRACA E LINHAS SUAVES NO SALDO FINAL */
                    .row-saldo-final, tr[id*="saldo-final"], tr[class*="saldo-final"] td, .row-saldo td {
                        background-color: #f4f5f7 !important; /* Mantém o cinza muito fraco do cabeçalho */
                        border-top: 1px solid #cccccc !important;   /* Linha horizontal muito fraca */
                        border-bottom: 1px solid #cccccc !important;/* Linha horizontal IBM */
                        padding: 10px 4px !important;
                        -webkit-print-color-adjust: exact !important;
                        print-color-adjust: exact !important;
                    }

                    /* Configurações de layout estrutural das tabelas */
                    table {
                        width: 100% !important;
                    ...
                    #capture-area, .folha-a4 * {
                        letter-spacing: 0.1px !important;
                    }
                `;
                clonedDoc.body.appendChild(style);

                /* GARANTIA EXTRA EM JAVASCRIPT: Varre as células do clone para remover o fundo cinza de onde diz "Saldo Inicial" */
                const cells = clonedDoc.querySelectorAll('.folha-a4 td');
                cells.forEach(td => {
                    if (td.textContent.includes("Saldo Inicial")) {
                        ...
                        if (row) {
                            ...
                            Array.from(row.children).forEach(child => {
                                ...
                                child.style.setProperty('border-top', '1px solid #cccccc', 'important');
                                child.style.setProperty('border-bottom', '1px solid #cccccc', 'important');
                            });
                        }
                    }
                });
        ...
        // 2. Processamento das Folhas de Dados (A4 Paisagem)
        ...
        // 3. Processamento das Páginas de Fotos (A4 Retrato)

...
async function novaFolha() {
    ...
    try {
        // 🔥 1. GUARDAR FOLHA ATUAL (se existir conteúdo)
        if (registros.length > 0 || fotos.length > 0) {
            ...
            await db.collection("folhas").doc(dadosParaSalvar.id).set(dadosParaSalvar);
        }

        // 🔥 2. OBTER SALDO FINAL
        const saldoAnterior = await obterUltimoSaldoFinal();

        // 🔥 3. LIMPAR TUDO (NOVA FOLHA)
        idFolhaAtual = null;
        registros = [];
        fotos = [];
        ...
        // 🔥 4. APLICAR SALDO AUTOMÁTICO
        if (saldoAnterior !== null) {
            document.getElementById('in-saldo-ini').value =
                saldoAnterior.toFixed(2).replace('.', ',');
        } else {
        ...
        alert("Nova folha criada com sucesso!");

    } catch (e) {
        console.error(e);
        alert("Erro ao criar nova folha");
    }
}
...
function update() {
    ...
    // Sempre que a tabela cresce, ajustamos o zoom
    ajustarZoom();
}

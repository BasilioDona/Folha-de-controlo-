...
async function gerarPDF() {
    ...
    try {
        // 1. Capturar todas as folhas de dados (A4 Paisagem) geradas dinamicamente
        const folhasA4 = area.querySelectorAll('.folha-a4');
        
        for (let i = 0; i < folhasA4.length; i++) {
            if (i > 0) {
                pdf.addPage('a4', 'landscape'); // Adiciona nova página se não for a primeira
            }
            const canvasFolha = await html2canvas(folhasA4[i], { 
                scale: 1.5, 
                useCORS: true 
            });
            pdf.addImage(canvasFolha.toDataURL('image/jpeg', 0.7), 'JPEG', 0, 0, 297, 210);
        }
